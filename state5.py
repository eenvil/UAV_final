...
def _load_latest_log_sequence():
    ...
    logs = glob.glob(os.path.join(base_dir, "tello_debug_state5.log"))
    if not logs:
        print("[state5] No log files found, using FALLBACK_SEQUENCE")
        return None

    latest = max(logs, key=os.path.getmtime)
    print(f"[state5] Using log file: {os.path.basename(latest)}")
    try:
        seq = _parse_log_to_sequence(latest)
        if seq is None:
            print("[state5] Failed to parse log, using FALLBACK_SEQUENCE")
        else:
            print(f"[state5] Parsed {len(seq)} steps from log.")
        return seq
    except Exception as e:
        print(f"[state5] Exception while parsing log: {e}")
        return None
